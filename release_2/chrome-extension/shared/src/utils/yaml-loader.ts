...
const DEFAULT_PRESETS: PlanPreset[] = [
...
interface PlanPresetsConfig {
  version?: string;
  last_updated?: string;
  schema?: {
    required_fields: string[];
    optional_fields: string[];
    validation_rules: Record<string, { min: number; max: number }>;
  };
  categories?: Array<{
    name: string;
    description: string;
...
class YamlConfigValidator {
  private config: PlanPresetsConfig;
  private errors: string[] = [];
  private warnings: string[] = [];

  constructor(config: PlanPresetsConfig) {
    this.config = config;
  }
...
let _cachedPresetsRaw: PlanPreset[] = [];
let _cachedPresetsView: readonly PlanPreset[] = [];
let _presetByNameMap: Map<string, PlanPreset> | null = null;
let _cacheBuilt: boolean = false;
let cacheTimestamp: number = 0;
const CACHE_TTL_MS = 60000;

const isNode = typeof process !== 'undefined' && process.versions && process.versions.node;

function _buildCaches(presets: PlanPreset[]): void {
...
export function loadPlanPresets(configPath?: string): PlanPreset[] {
  const now = Date.now();
  ...
  if (isNode) {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        const config = yaml.load(fileContents) as PlanPresetsConfig;
...
export async function loadPlanPresetsAsync(configPath?: string): Promise<PlanPreset[]> {
  const now = Date.now();
...
export function getPlanPresetByName(presets: PlanPreset[], name: string): PlanPreset | undefined {
...
export function getPresetsByCategory(presets: PlanPreset[], category: string): PlanPreset[] {
  const normalizedCategory = category.toLowerCase().trim();
  return presets.filter(preset =>
    preset.category && preset.category.toLowerCase() === normalizedCategory
  );
}
