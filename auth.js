...
const { AuditLogger, EVENT_TYPES } = require('./gov-modules/audit-logger');
const { RBAC, ROLES } = require('./gov-modules/rbac');
const dataMasking = require('./gov-modules/data-masking');
...
class AuthManager {
  constructor() {
    ...
    this.mfaStore = new Map(); // å¤šå› ç´ è®¤è¯å­˜å‚¨
    this.sessionStore = new Map(); // ä¼šè¯ç®¡ç†
    this.auditLogger = new AuditLogger(); // å®¡è®¡æ—¥å¿—
    this.rbac = new RBAC(); // æƒé™ç®¡ç†
    this.loginAttempts = new Map(); // ç™»å½•å¤±è´¥æ¬¡æ•°é™åˆ¶
  ...
  initializeRBAC() {
    // ä¸ºçŽ°æœ‰ç”¨æˆ·åˆ†é…é»˜è®¤è§’è‰²
    Object.values(this.users).forEach((user) => {
      if (!this.rbac.users.has(user.id)) {
        const role = user.role || ROLES.USER;
        this.rbac.createUser(user.id, user.displayName, role);
      }
    });
  }
  ...
  generateMFA(userId) {
    ...
    this.mfaStore.set(userId, { code: mfaCode, expiresAt });
    return mfaCode;
  }

  verifyMFA(userId, code) {
    const stored = this.mfaStore.get(userId);
    if (!stored) {
      return { success: false, error: 'No MFA code requested' };
    }
  ...
  getUserById(userId) {
    const email = this.userIdIndex.get(userId);
  ...
  checkLoginAttempts(email) {
    ...
    if (attempts >= 5) {
      return { blocked: true, remainingTime: 15 * 60 * 1000 }; // 15åˆ†é’Ÿå°é”
    }
  ...
  async register(email, password, displayName, role = ROLES.USER) {
    const cleanEmailForLog = sanitizeEmail(email);
    logger.info('Attempting user registration', {
      email: dataMasking.maskEmail(cleanEmailForLog),
    ...
    try {
      ...
      if (!emailValidation.valid) {
        ...
        this.auditLogger.log({
          userId: null,
          username: null,
          eventType: EVENT_TYPES.USER_CREATE,
          level: 'warn',
          resource: 'auth',
          action: 'register_failed',
          result: 'failed',
          errorMessage: emailValidation.error,
          details: { email: dataMasking.maskEmail(cleanEmailForLog) },
        });
      ...
      if (this.users[cleanEmail]) {
        logger.warn('Registration failed - user already exists', {
          email: dataMasking.maskEmail(cleanEmail),
        });
      ...
      this.auditLogger.logLogin(user.id, user.displayName, 'success', null, { registration: true });
      ...
      logger.error('Unexpected error during registration', error, {
        email: dataMasking.maskEmail(sanitizeEmail(email)),
      });
      return { success: false, error: 'An unexpected error occurred during registration' };
    }
  }

  async login(email, password, requireMFA = false) {
    const cleanEmailForLog = sanitizeEmail(email);
    logger.info('Attempting user login', { email: dataMasking.maskEmail(cleanEmailForLog) });

    try {
      // æ£€æŸ¥ç™»å½•å¤±è´¥æ¬¡æ•°
      const attemptCheck = this.checkLoginAttempts(cleanEmailForLog);
      ...
      if (!user) {
        logger.warn('Login failed - user not found', { email: dataMasking.maskEmail(cleanEmail) });
        this.incrementLoginAttempts(cleanEmail);
        this.auditLogger.logLogin(null, null, 'failed', null, { reason: 'user_not_found' });
      ...
      if (!passwordValid) {
        ...
        this.incrementLoginAttempts(cleanEmail);
        this.auditLogger.logLogin(user.id, user.displayName, 'failed', null, {
      ...
      // é‡ç½®ç™»å½•å¤±è´¥æ¬¡æ•°
      this.resetLoginAttempts(cleanEmail);

      // æ£€æŸ¥æ˜¯å¦éœ€è¦å¤šå› ç´ è®¤è¯
      if (user.mfaEnabled || requireMFA) {
        const mfaCode = this.generateMFA(user.id);
        logger.info('MFA code generated for user', { userId: user.id });
        // åœ¨å®žé™…ç”Ÿäº§ä¸­ï¼Œè¿™é‡Œä¼šå‘é€é‚®ä»¶æˆ–çŸ­ä¿¡
        console.log(`[MFA CODE] For ${dataMasking.maskEmail(cleanEmail)}: ${mfaCode}`);
        return {
          success: true,
          requiresMFA: true,
          mfaToken: cryptoUtils.generateSecureId(32),
      ...
      this.auditLogger.logLogin(user.id, user.displayName, 'success', null);
  ...
  updateProfilePicture(email, picture) {
    const cleanEmail = sanitizeEmail(email);
    const user = this.users[cleanEmail];
    const validImage = /^data:image\/(png|jpe?g|webp|gif);base64,[a-z0-9+/=]+$/i;
    if (!user) return { success: false, error: 'User not found' };
    if (typeof picture !== 'string' || !validImage.test(picture) || picture.length > 4 * 1024 * 1024) {
      return { success: false, error: 'Choose a PNG, JPEG, WebP, or GIF image smaller than 3 MB.' };
    }
    ...
    if (code && code.startsWith('mock_github_')) {
      ...
      return {
        ...
        githubUser: {
          id:
            'mock-github-id-' +
            crypto.createHash('md5').update(mockEmail).digest('hex').substring(0, 12),
          email: mockEmail,
          login: mockLogin,
