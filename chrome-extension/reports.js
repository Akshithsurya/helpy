...
function periodDescription(days) {
  if (days === 1) return 'Tracked today';
  if (days === 7) return 'Tracked in the last 7 days';
  return `Tracked in the last ${days} days`;
}
...
async function updateReport(period) {
  ...
  document.getElementById('periodLabel').textContent = periodDescription(period);
  document.getElementById('reportUpdatedAt').textContent = `Updated ${new Date().toLocaleTimeString([], {
    hour: 'numeric',
    minute: '2-digit',
  })}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('refreshReport').addEventListener('click', () => updateReport(currentPeriod));
