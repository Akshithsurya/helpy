...
class NotificationManager {
  constructor(logger, taskManager) {
    ...
    this._saveNotificationsDataSync = this._saveNotificationsDataSyncReal.bind(this);
    this.debouncedSave = inlineDebounce(this._saveNotificationsDataSync, 500);
    this._saveNotificationsData = this.debouncedSave;
  ...
  _registerExitHooks() {
    try {
      if (typeof process !== 'undefined' && process && typeof process.on === 'function') {
        const doFlush = () => this.flushPendingSaves();
  ...
  flushPendingSaves() {
    try { this.debouncedSave.flush(); } catch (_) {}
    try { this._saveNotificationsDataSync(); } catch (_) {}
  }
  ...
  _saveNotificationsDataSyncReal() {
    try {
      const filePath = getDataFilePath(NOTIFICATIONS_DATA_PATH);
  ...
  createNotification(notificationData) {
    try {
      ...
      if (this.notificationsData.notifications.length > 500) {
        this.notificationsData.notifications.length = 500;
      }
  ...
  markAllAsRead() {
    try {
      ...
      const arr = this.notificationsData.notifications;
      for (let i = 0; i < arr.length; i++) {
        arr[i].read = true;
        arr[i].updatedAt = nowStr;
      }
      this._saveNotificationsData();
      return { success: true };
    } catch (error) {
  ...
  getNotificationStats() {
    ...
    const allNotifs = this.notificationsData.notifications;
    let todayCount = 0;
    for (let i = 0; i < allNotifs.length; i++) {
      if (new Date(allNotifs[i].createdAt).toDateString() === today) todayCount++;
    }
  ...
  startDeadlineChecker() {
    ...
    this.deadlineCheckInterval = setInterval(() => this._checkDeadlines(), 60000);
