...
// Sentinel value passed to JsonValue::serialize() to request compact output.
// NOTE: relying on a magic sentinel is fragile; prefer a dedicated
// serializeCompact() overload on JsonValue if that class is open for change.
...
//  allocation failure. The exception is swallowed silently today; wire up
//  debug logging here (e.g. via spdlog/LOG_DEBUG) if parse failures need
//  diagnosing in the field.
[[nodiscard]] inline std::optional<JsonValue> tryFromJson(std::string_view json) noexcept {
    try {
        return JsonUtils::parseJson(std::string{json});
    } catch (...) {
        return std::nullopt;
