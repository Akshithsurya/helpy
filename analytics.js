...
class AnalyticsManager {
  ...
  calculateProductivityScore(date = new Date()) {
    try {
      const dayKey = this._getDayKey(date);
      const stats = this.analyticsData.dailyStats[dayKey] || {
        focusSessions: 0,
        totalFocusMinutes: 0,
        tasksCompleted: 0,
        distractions: 0,
      ...
      // Score calculation: Base score out of 100
      // 1. Focus time contribution (up to 40 pts, maxing out at 120 mins)
      const focusScore = Math.min(40, (stats.totalFocusMinutes / 120) * 40);
      // 2. Tasks completed contribution (up to 40 pts, 8 pts per task up to 5 tasks)
      const taskScore = Math.min(40, (stats.tasksCompleted || 0) * 8);
      // 3. Focus sessions consistency (up to 20 pts, 5 pts per session)
      const sessionScore = Math.min(20, (stats.focusSessions || 0) * 5);
      // 4. Distractions penalty (-5 pts per distraction)
      const penalty = Math.min(30, (stats.distractions || 0) * 5);

      const totalScore = Math.max(
        0,
        Math.min(100, Math.round(focusScore + taskScore + sessionScore - penalty))
      );
      return {
        score: totalScore,
        breakdown: { focusScore, taskScore, sessionScore, penalty },
      };
    } catch (error) {
  ...
  getVelocityMetrics(days = 7) {
    try {
      const report = this.getProductivityReport(days);
      if (!report || !report.summary) {
        return { averageDailyTasks: 0, totalFocusHours: 0, completionRate: 0 };
      }

      const totalTasks = report.summary.totalTasksCompleted || 0;
      const totalMinutes = report.summary.totalFocusMinutes || 0;
      const averageDailyTasks = parseFloat((totalTasks / days).toFixed(1));
      const totalFocusHours = parseFloat((totalMinutes / 60).toFixed(1));
      ...
      this.logger.error('Error getting velocity metrics:', error);
      return { averageDailyTasks: 0, totalFocusHours: 0, completionRate: 0 };
    }
  }
}
