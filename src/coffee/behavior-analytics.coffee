###
BehaviorAnalytics v2.3.0
lightweight, privacy-friendly event tracker with persistence,
trend analysis, and event emitter hooks.

Changelog (v2.3.0):
  - Add collab_* to default whitelist
  - Add privacy optIn/optOut + removeEventsBefore / clearEventsBefore
  - Add newSession, registerEventTypes, unregisterEventType
  - Add off() emitter counterpart, setLogger
  - Defensive unfreeze in _loadHistory backfill
  - CSV formula-injection mitigation
  - Configurable detectProductivityDrop threshold
  - trackError captures stack
  - trackBatch accepts object form {type, data}
  - getSessionStats, getTopEventTypes, getEventCount helpers
  - Safer max-hour computation (no spread on empty)
  - MusicAnalytics.autoTrack returns { unsubs, destroy }
###
...
# ── CSV helper (RFC 4180 + formula-injection mitigation) ────────────────────
...
_coerceField = (value, rule) ->
  switch rule
    ...
    when 'clamped01'  then if _isNumber(value) then _clamp(value, 0, 1) else undefined
    when 'boolean'    then !!value
    when 'floorInt'   then if _isNumber(value) then Math.floor(value) else undefined
    else undefined
...
class BehaviorAnalytics
  ...
  constructor: (@store = null, opts = {}) ->
    ...
    @_optedIn        = opts.optedIn isnt false   # privacy: default opt-in
  ...
  optOut: ->
    @_optedIn = false
    @flush()   # persist whatever is queued before going dark
    @
  ...
  _resolveLogger: (candidate) ->
    if _isPlainObject(candidate) and
      ...
      candidate
    else if candidate is 'noop'
      NOOP_LOGGER
    else
      console

  setLogger: (logger) ->
    @logger = @_resolveLogger logger
    @
  ...
  _assertAlive: (methodName) ->
    if @_destroyed
      @logger.debug "[BehaviorAnalytics] #{methodName} after destroy — ignored"
      false
    else
      true
  ...
  registerEventTypes: (types) ->
    return 0 unless Array.isArray types
    added = 0
    for t in types when _isString(t) and t.length > 0 and not @_allowedEventTypes.has t
      @_allowedEventTypes.add t
      added++
    added

  unregisterEventType: (type) ->
    return false unless _isString(type) and @_allowedEventTypes.has type
    @_allowedEventTypes.delete type
    true
  ...
  _uniqueId: (prefix) ->
    ...
    "#{prefix}_#{@_clock.now()}_#{@_idCounter}_#{rnd}"

  newSession: (id) ->
    return null unless @_assertAlive 'newSession'
    @currentSession = if _isString(id) and id.length > 0 then id else @_uniqueId 'session'
    @trackEvent 'session_start', { sessionId: @currentSession, reason: 'rotated' }
    @currentSession
  ...
  _isoTimestamp: (d) ->
    ...
    "T#{p(d.getUTCHours())}:#{p(d.getUTCMonth() + 1)}-#{p(d.getUTCDate())}T#{p(d.getUTCHours())}:#{p(d.getUTCMinutes())}:#{p(d.getUTCSeconds())}.#{p(d.getUTCMilliseconds(), 3)}Z".replace(/^.*?T/, (m) ->
      # safeguard: rebuild cleanly
      "#{d.getUTCFullYear()}-#{p(d.getUTCMonth() + 1)}-#{p(d.getUTCDate())}T"
    )
    # NB: the line above was an awkward attempt; use the canonical form:
    canonical = "#{d.getUTCFullYear()}-#{p(d.getUTCMonth() + 1)}-#{p(d.getUTCDate())}" +
  ...
  _loadHistory: ->
    ...
    try
      ...
      # Backfill `day` for older events; defensively handle frozen ones
      for e, i in @events
        continue if e.day?
        dayKey = @_localDayKey new Date e.time
        if Object.isFrozen e
          @events[i] = Object.assign {}, e, { day: dayKey }
        else
          e.day = dayKey
  ...
  flush: ->
    return unless @_assertAlive 'flush'
    @_cancelSave()
    @_persist()
    @_eventsSinceFlush = 0
  ...
  off: (eventName, handler) ->
    return false unless eventName in VALID_EMITTER_EVENTS
    list = @_listeners[eventName]
    return false unless list?.length and _isFunction handler
    idx = list.indexOf handler
  ...
  _buildEvent: (eventType, data) ->
    return null unless @_assertAlive '_buildEvent'
    ...
    # Reject payloads that are too large to persist safely
    try
      if JSON.stringify(sanitized).length > MAX_PAYLOAD_BYTES
        @logger.warn "[BehaviorAnalytics] event '#{eventType}' payload exceeds #{MAX_PAYLOAD_BYTES} bytes — truncated"
        sanitized = { _truncated: true, _originalType: typeof data }
    catch
      sanitized = { _unserializable: true }
  ...
  trackBatch: (eventPairs) ->
    ...
    return [] unless @_optedIn
    results = []
    for pair in eventPairs
      type = null
      data = {}
      if Array.isArray(pair) and pair.length > 0
        [type, data] = pair
        data = {} unless _isPlainObject data
      else if _isPlainObject(pair) and _isString(pair.type)
        type = pair.type
        data = if _isPlainObject(pair.data) then pair.data else {}
      else
        continue
  ...
  removeEventsBefore: (time) ->
    return 0 if @_destroyed or not _isNumber(time)
    kept = []
    removed = 0
    for e in @events
      if e?.time? and e.time < time then removed++ else kept.push e
    @events = kept
    @_scheduleSave() if removed > 0
    removed

  clearEventsBefore: (days) ->
    return 0 unless _isNumber(days) and days > 0
    @removeEventsBefore @_clock.now() - Math.floor(days) * MS_PER_DAY
  ...
  getUsageStatistics: (days = DEFAULT_LOOKBACK_DAYS) ->
    ...
    {
      totalEvents, planActions, uiInteractions
      sessions: sessions.size
      popularTimes: hourHist
      invalidHourCount: invalidHour
      byType
    }

  getSessionStats: (days = DEFAULT_LOOKBACK_DAYS) ->
    windowDays = if _isNumber(days) and days > 0 then Math.floor(days) else DEFAULT_LOOKBACK_DAYS
    cutoff     = @_clock.now() - windowDays * MS_PER_DAY
    sessions   = new Map()
    for e in @events when e?.time? and e.time >= cutoff and _isString e.sessionId
      sessions.set e.sessionId, (sessions.get(e.sessionId) ? 0) + 1
    counts = Array.from sessions.values()
    total  = counts.reduce ((a, b) -> a + b), 0
    avg    = if counts.length then total / counts.length else 0
    max    = if counts.length then Math.max(...counts) else 0
    min    = if counts.length then Math.min(...counts) else 0
    { uniqueSessions: sessions.size, totalEvents: total, avgEventsPerSession: avg, maxEventsInSession: max, minEventsInSession: min }

  getTopEventTypes: (limit = 5, days) ->
    limit = if _isNumber(limit) and limit > 0 then Math.floor limit else 5
    cutoff = if _isNumber(days) and days > 0 then @_clock.now() - Math.floor(days) * MS_PER_DAY else 0
    counts = {}
    for e in @events when e?.time? and e.time >= cutoff
  ...
  getEventCount: (type, days) ->
    return 0 unless _isString type
    cutoff = if _isNumber(days) and days > 0 then @_clock.now() - Math.floor(days) * MS_PER_DAY else 0
    n = 0
    for e in @events when e?.type is type and e?.time? and e.time >= cutoff then n++
    n
  ...
  getPersonalizedSuggestions: (opts = {}) ->
    ...
    for e in @events
      if 0 <= e.hour < HOURS_PER_DAY then hourCounts[e.hour]++
      if e.type is 'plan_action' and _isPlainObject(e.data) and _isString(e.data.action)
        planTotal++
        planComplete++ if e.data.action is 'complete'

    # safer max: explicit loop, handles all-zero gracefully
    maxCount = -1
    for c in hourCounts then maxCount = c if c > maxCount
    maxCount = 0 if maxCount < 0
  ...
  detectProductivityDrop: (windowDays = DEFAULT_DROP_WINDOW_DAYS, threshold = DEFAULT_DROP_THRESHOLD) ->
    windowDays = if _isNumber(windowDays) and windowDays > 0 then Math.floor(windowDays) else DEFAULT_DROP_WINDOW_DAYS
    threshold  = if _isNumber(threshold)  and 0 < threshold  < 1 then threshold  else DEFAULT_DROP_THRESHOLD
    now       = @_clock.now()
    ...
    isDrop   = previous > 0 and ratio < threshold
    { isDrop, recentCount: recent, previousCount: previous, ratio, threshold }
...
class MusicAnalytics
  ...
  autoTrack: (player) ->
    ...
    destroy = ->
      for u in unsubs when _isFunction u
        try u() catch then undefined
      unsubs.length = 0
      return
...
class FocusAnalytics
  ...
  endSession: ->
    ...
    # Guard against missing _sessionStart (defensive)
    unless _isNumber @_sessionStart
      @_resetState()
      return null
    ...
    activeDuration = 0 if not _isNumber(activeDuration) or activeDuration < 0
  ...
  getFocusStats: (days = DEFAULT_FOCUS_STATS_DAYS) ->
    ...
    for e in @analytics.events when e?.time >= cutoff and e.type is 'focus_session_end'
      ...
      duration = 0 unless _isNumber(duration) and duration >= 0
