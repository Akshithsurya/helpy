...
function initAppDependencies(dependencies = {}) {
  ...
  if (dependencies.focusSessionManager) {
    focusSessionManager = dependencies.focusSessionManager;
  }
}
...
function respondPlanError(res, status, error) {
...
function getRequiredFocusPlanManager() {
  if (!focusPlanManager) {
    throw new Error('Focus plan manager is unavailable');
  }

  return focusPlanManager;
}
...
expressApp.post('/api/focus-plan', (req, res) => {
  try {
    ...
    const body = req.body || {};
    if (!body.title) {
      return respondPlanError(res, 400, 'Plan title is required');
    }
    ...
    respondPlanError(res, 400, error.message || 'Invalid focus plan');
  }
});

expressApp.get('/api/focus-plans/history', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) {
      return respondUnauthorized(res);
    }

    const manager = getRequiredFocusPlanManager();
    ...
    console.error('[ERROR] handling /api/focus-plans/history GET:', error);
    return respondPlanError(res, 400, error.message || 'Failed to load focus plan history');
  }
});

expressApp.get('/api/focus-plans/stats', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) {
      return respondUnauthorized(res);
    }
    ...
    console.error('[ERROR] handling /api/focus-plans/stats GET:', error);
    return respondPlanError(res, 400, error.message || 'Failed to load focus plan statistics');
  }
});

expressApp.get('/api/focus-plans/templates', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) {
      return respondUnauthorized(res);
    }
    ...
    console.error('[ERROR] handling /api/focus-plans/templates GET:', error);
    return respondPlanError(res, 400, error.message || 'Failed to load focus plan templates');
  }
});

expressApp.post('/api/focus-plans/templates', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) {
      return respondUnauthorized(res);
    }
    ...
    console.error('[ERROR] handling /api/focus-plans/templates POST:', error);
    return respondPlanError(res, 400, error.message || 'Failed to create focus plan template');
  }
});

expressApp.delete('/api/focus-plans/templates/:templateId', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) {
      return respondUnauthorized(res);
    }
    ...
    const manager = getRequiredFocusPlanManager();
    const deleted = manager.deleteTemplate(req.params.templateId);
    if (!deleted) {
      return respondPlanError(res, 404, 'Focus plan template not found');
    }
    ...
    console.error('[ERROR] handling /api/focus-plans/templates DELETE:', error);
    return respondPlanError(res, 400, error.message || 'Failed to delete focus plan template');
  }
});
...
expressApp.get('/api/reports', (req, res) => {
  try {
    ...
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    const reportEntries = tabHistory
      .map((entry) => {
        const startTime = Number(entry.startTime);
        const endTime = Number(entry.endTime) || startTime + Number(entry.duration || 0);
        const overlapStart = Math.max(startTime, cutoff);
        const overlapEnd = Math.min(endTime, Date.now());

        if (!entry.domain || !Number.isFinite(startTime) || overlapEnd <= overlapStart) return null;
        return { domain: entry.domain, duration: overlapEnd - overlapStart };
      })
      .filter(Boolean);

    // Calculate domain time distribution using only the portion in the report period.
    const domainTime = {};
    reportEntries.forEach((entry) => {
      if (!domainTime[entry.domain]) domainTime[entry.domain] = { totalTime: 0, visits: 0 };
      domainTime[entry.domain].totalTime += entry.duration;
      domainTime[entry.domain].visits += 1;
    });
    ...
    const totalTime = reportEntries.reduce((sum, entry) => sum + entry.duration, 0);
    ...
    res.json({
      ...
      reports: {
        timeDistribution,
        totalTime,
        tabSwitchCount: reportEntries.length,
...
expressApp.get('/auth/mock', (req, res) => {
  ...
  const capitalizedProvider = provider
...
// ── Block rules CRUD (auth required) ─────────────────────────────────────────

expressApp.get('/api/block-rules', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) return respondUnauthorized(res);
    res.json({ success: true, rules: blockScheduler.getRules(), bridge: getBridgeMetadata() });
  } catch (error) {
    console.error('[ERROR] /api/block-rules GET:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

expressApp.post('/api/block-rules', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) return respondUnauthorized(res);
    if (focusSessionManager?.isEditsLocked()) {
      return res.status(423).json({
        success: false,
        error: 'Blocklist edits are locked for the active strict focus session.',
      });
    }
    const rule = blockScheduler.createRule(req.body || {});
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('block-rules-updated', blockScheduler.getRules());
    }
    res.status(201).json({ success: true, rule, bridge: getBridgeMetadata() });
  } catch (error) {
    console.error('[ERROR] /api/block-rules POST:', error);
    res.status(400).json({ success: false, error: error.message });
  }
});

expressApp.put('/api/block-rules/:id', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) return respondUnauthorized(res);
    if (focusSessionManager?.isEditsLocked()) {
      return res.status(423).json({
    ...
    const rule = blockScheduler.updateRule(req.params.id, req.body || {});
    if (!rule) return res.status(404).json({ success: false, error: 'Rule not found' });
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('block-rules-updated', blockScheduler.getRules());
    }
    res.json({ success: true, rule, bridge: getBridgeMetadata() });
  } catch (error) {
    console.error('[ERROR] /api/block-rules/:id PUT:', error);
    res.status(400).json({ success: false, error: error.message });
  }
});

expressApp.delete('/api/block-rules/:id', (req, res) => {
  try {
    if (!isAuthorizedBridgeRequest(req)) return respondUnauthorized(res);
    if (focusSessionManager?.isEditsLocked()) {
      return res.status(423).json({
    ...
    const deleted = blockScheduler.deleteRule(req.params.id);
    if (!deleted) return res.status(404).json({ success: false, error: 'Rule not found' });
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('block-rules-updated', blockScheduler.getRules());
    }
    res.json({ success: true, deleted: true, bridge: getBridgeMetadata() });
  } catch (error) {
    console.error('[ERROR] /api/block-rules/:id DELETE:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

// ── Block state (no auth — extension polls this) ──────────────────────────────
// Returns the union of all currently-active blocked domains.
// Also merges the always-on blockedDomains list from settings if blockingEnabled.
expressApp.get('/api/block-state', (req, res) => {
  try {
    const origin = req.headers.origin;
    if (origin && !isOriginAllowed(origin)) return respondForbidden(res);

    const settings = settingsManager.getSettings();
    const schedulerState = blockScheduler.getBlockState();

    // Merge always-on blocked domains from settings when master switch is on
    let blockedDomains = [...schedulerState.blockedDomains];
    if (settings.blockingEnabled && Array.isArray(settings.blockedDomains)) {
      for (const d of settings.blockedDomains) {
        if (d && !blockedDomains.includes(d)) blockedDomains.push(d);
      }
    }

    const session = focusSessionManager ? focusSessionManager.getState() : null;
    // Local HTTP polling is used because this project already has a loopback Express bridge.
    // It avoids native-host installation while keeping all state on the device.
    const active =
      settings.blockingEnabled && blockedDomains.length > 0 && (!session || session.blockingActive);
    res.json({
      success: true,
      active,
      ...
      activeRules: schedulerState.activeRules,
      session,
      bridge: getBridgeMetadata(),
    });
  } catch (error) {
    console.error('[ERROR] /api/block-state GET:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

expressApp.post('/api/blocked-attempt', (req, res) => {
  try {
    const origin = req.headers.origin;
    if (origin && !isOriginAllowed(origin)) return respondForbidden(res);
    focusSessionManager?.reportBlockedAttempt(req.body?.domain);
    res.status(204).end();
  } catch (error) {
    console.error('[ERROR] /api/blocked-attempt POST:', error);
